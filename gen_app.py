...
function renderGroups(m,data,key){
  ...
  data.forEach(b=>{
    ...
    else { const g = key==="author"?(b.author||"—") : key==="shelf"?(b.shelf||"Unshelved") : topGenre(b.genre); (groups[g]=groups[g]||[]).push(b); }
  });
  ...
  const label={author:"authors",genre:"genres",collection:"collections",shelf:"shelves"}[key]||"groups";
  ...
  document.getElementById("sheetbody").innerHTML=`
    ...
    <div class="dtop">
      ...
      <div class="dmeta">
        ...
        <div>${b.genre?`<span class="pill">${esc(b.genre)}</span>`:""}${b.year?`<span class="pill">${esc(b.year)}</span>`:""}${b.shelf?`<span class="pill">${esc(b.shelf)}</span>`:""}${(b.collections||[]).map(c=>`<span class="pill coll">${esc(c)}</span>`).join("")}</div>
      </div>
    </div>
