...
function openReader(eid,title){
  ...
  // Fetch the epub as bytes and hand epub.js the ArrayBuffer — passing the URL directly
  // makes epub.js treat the (extension-less) path as an UNPACKED directory and it never unzips.
  fetch(syncURL()+"/ebook/"+eid,{cache:"force-cache"})
    ...
    .then(buf=>{
      ...
      _rendition.on("relocated",loc=>{
        try{ if(loc&&loc.start){ localStorage.setItem("lib_pos_"+eid,loc.start.cfi); updateReaderPage(loc); } }catch(e){}
      });
      return _rendition.display(pos||undefined);
    })
    .then(()=>{
      rload.style.display="none";
      // build locations for the page count + % (async; harmless if it fails)
      try{ _rbook.locations.generate(1400).then(()=>{ try{ if(_rendition&&_rendition.currentLocation){ updateReaderPage(_rendition.currentLocation()); } }catch(e){} }); }catch(e){}
    })
...
function readerNav(dir){
  const now=Date.now(); if(now-_navAt<420) return;   // tiny delay: collapses touch+click double-fire and rate-limits
  _navAt=now;
  if(_rendition){ dir<0?_rendition.prev():_rendition.next(); }
}
function updateReaderPage(loc){
  try{
    if(!loc||!loc.start) return;
    const el=document.getElementById("rpage"); if(!el) return;
    let txt="";
    if(_rbook && _rbook.locations && _rbook.locations.length()){
      let cur=loc.start.location; if(cur==null) cur=_rbook.locations.locationFromCfi(loc.start.cfi);
      const tot=_rbook.locations.total;
      let p=loc.start.percentage; if(p==null||isNaN(p)){ try{p=_rbook.locations.percentageFromCfi(loc.start.cfi);}catch(e){} }
      txt="page "+((cur|0)+1)+" of "+tot+(p!=null&&!isNaN(p)?"   ·   "+Math.round(p*100)+"%":"");
    } else if(loc.start.displayed && loc.start.displayed.total){
      txt="page "+loc.start.displayed.page+" of "+loc.start.displayed.total;
    }
    el.textContent=txt;
  }catch(e){}
}
