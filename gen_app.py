...
/* shelf visualization */
.shelfwrap{margin-bottom:20px}
.shelfhdr{font-size:12px;font-weight:700;color:var(--sage-dark);text-transform:uppercase;letter-spacing:.4px;margin-bottom:4px}
.shelfhdr span{color:var(--muted);font-weight:600}
.shelf{display:flex;flex-wrap:wrap;align-items:flex-end;gap:2px;padding:12px 8px 0;
  background:linear-gradient(180deg,rgba(0,0,0,.03),transparent);border-bottom:9px solid #6b4e34;
  border-radius:3px;box-shadow:inset 0 -12px 14px -12px rgba(0,0,0,.35)}
.spine{writing-mode:vertical-rl;text-orientation:mixed;color:#fff;font-size:10px;font-weight:700;line-height:1.05;
  padding:9px 3px;border-radius:3px 3px 0 0;cursor:pointer;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;
  box-shadow:1px 0 2px rgba(0,0,0,.25);border-top:2px solid rgba(255,255,255,.18)}
.spine:active{filter:brightness(1.15)}
</style>
</head>
<body>
<header>
...
function renderWishlist(m){
  ...
  const addBar = `<div style="display:flex;gap:8px;margin:0 0 12px">
    <button class="imp" style="margin:0;flex:1" onclick="openAddWish()">＋ Add a book</button>
    <button class="imp sec" style="margin:0;flex:none;padding:13px 16px" onclick="shareWishlist()">🔗 Share</button></div>`;
  if(!wishlist.length){
...
function renderShelf(m,data){
  document.getElementById("viewnote").textContent = data.length+" spines";
  ...
  m.innerHTML = names.map(n=>`<div class="shelfwrap">
    ...
    <div class="shelf">${groups[n].map(b=>{
      const len=(b.title||"").length; const w=Math.max(20,Math.min(36,17+Math.round(len/3.5)));
      const h=120+((len*13)%56); const light=(topGenre(b.genre)==="Children's & YA");
      return `<div class="spine" onclick="openBook(${b.id})" style="width:${w}px;height:${h}px;background:${genreColor(b.genre)}${light?';color:#3a2a12':''}" title="${esc(b.title)} — ${esc(b.author)}">${esc(b.title)}</div>`;
    }).join("")}</div></div>`).join("");
}
